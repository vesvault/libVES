...
libVES.prototype = {
//    apiUrl: 'https://www.vesvault.com/api/',
    apiUrl: '/api/v1/',
//    e2e: ['signal'],
    keyAlgo: 'RSA',
	...
	return new Promise(function(resolve,reject) {
		...
		switch(xhr.readyState) {
			...
			if (xhr.response && typeof(xhr.response) == 'object') {
				...
				if (errs.length) {
				    if (optns && optns.onerror) resolve(optns.onerror(errs));
				    else reject(errs[0]);
				}
			    }
	    ...
	    else if (this.user) xhr.setRequestHeader('Authorization','Basic ' + btoa(this.user + ':' + this.passwd));
    ...
    connect: function(optns) {
	var self = this;
	for (var k in optns) this[k] = optns[k];
	if (this.token != null || (this.user != null && this.passwd != null)) return this.get('me').then(function() {
	    return self;
	});
	return Promise.reject("libVES: connect(): token or user + password required");
    },
    external: function(domain,extid) {
    },
    me: function() {
	var self = this;
